...
#if defined(NDEBUG)
# error "Gurtaj cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xecff38ffb01d2b9e2bb1dbdc6f5491a8ef6ed16cba61e13059acc1f10b6baa03");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gurtaj: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gurtaj Signed Message:\n";
...
{
    // Gurtaj: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 12000); // Gurtaj: 12k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Gurtaj: 24 hours
static const int64 nTargetSpacing = 1 * 60; // Gurtaj: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gurtaj: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gurtaj: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6cae1bf2cb1896e405c0403869615fa337c841dadd32d5e8912e5b8e612a019a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "$44 BILLION DOLLAR TECH DEAL USA AND CHINA";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 4000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b890963b4cad7d0728ab4cc73ce4310ff2b3eb048334c4de9e18515da703993cfab3118744208cfa677fa7010fc84e20f3521e8f2edeca4015e64b1207a069a8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525222812;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086751882;

        if (fTestNet)
        {
            block.nTime    = 1525222796;
            block.nNonce   = 385924462;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x49fdc8e4bd3c4a489bccebb43fa721237fb221f676cd0d60f6e3030e78708476"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc8, 0xd6, 0xdf};// Gurtaj: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GurtajMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GurtajMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GurtajMiner(CWallet *pwallet)
{
    printf("GurtajMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gurtaj-miner");
    ...
    try { loop {
        ...
        printf("Running GurtajMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GurtajMiner, pwallet));
}
